...
MonsterCard::MonsterCard(string name, string effect, int attackingPoints, int protectivePoints, unsigned int rarity)
...
ostream &MonsterCard::operator<<(ostream &os) const {
    ...
    os << to_string(this->protectivePoints) + NEW_LINE;

    return os;
}

void MonsterCard::setAttackingPoints(unsigned int points) {
...
string MonsterCard::getObjectType() const {
    return "monsterCard.hpp";
}

Card *MonsterCard::clone() {
    return new MonsterCard(this->getName(), this->getEffect(), this->getAttackingPoints(), this->getProtectivePoints(), this->getRarity());
}
